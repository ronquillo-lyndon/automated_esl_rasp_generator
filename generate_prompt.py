...
def prompt(langauge, topic, number_of_paragraph, range : Range, number_of_questions):
    range_f, range_t = "", ""
    def _check_range(r):
        range_temp = ""
    ...
    return f"Generate English essay about {topic} in {number_of_paragraph} paragraph format, From {range_f} to {range_t} – Each level should have their own essay. So, I can test my {langauge} speaking and Reading skills. Ensure to add questions in each essay to practice and improve my comprehension and ability to formulate answers in {langauge}. The Questions part should be divided into two section Comprehension and Speaking, Each section should have {number_of_questions} questions"

if __name__ == "__main__":
    range = Range(('A', 1), ('B', 1))
    print(prompt("English", "Shabu", 3, range, 2))
